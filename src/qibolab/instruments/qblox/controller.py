...
class QbloxController(AbstractPlatform):
    ...
    def play(self, qubits: dict, sequence: PulseSequence, options: ExecutionParameters):
        return self._execute_pulse_sequence(
            qubits=qubits,
            sequence=sequence,
            options=options,
        )
    ...
    # DesignPlatform interface
    def sweep(self, qubits: dict, sequence: PulseSequence, *sweepers, options: ExecutionParameters):
        return self._sweep(qubits=qubits, sequence=sequence, sweepers=list(sweepers), options=options)

    # AbstractPlatform interface
    def execute_pulse_sequence(self, *args, **kwargs):
        pass
                ...
                if all(s.parameter in rt_sweepers for s in sweepers):
                    ...
                    if num_bins < 2**17:
                        ...
                        result = self._execute_pulse_sequence(
                            qubits=qubits, sequence=sequence, options=options, sweepers=sweepers
                        )
                        for pulse in sequence.ro_pulses:
                            results[pulse.id] += result[pulse.serial]
                            results[pulse.qubit] = results[pulse.id]
